...
impl StripePaymentPlugin {
    ...
    fn verify_signature(&self, raw_payload: &[u8], sig_header: &str) -> Result<()> {
        ...
        for segment in sig_header.split(',') {
            if let Some(v) = segment.strip_prefix("t=")  { ts = Some(v); }
            if let Some(v) = segment.strip_prefix("v1=") { v1_sigs.push(v); }
        }
...
impl PaymentPlugin for StripePaymentPlugin {
    ...
    async fn create_invoice(&self, request: CreateInvoiceRequest) -> Result<Invoice> {
        if !self.config.configured {
            bail!("Stripe plugin not configured: {:?}", self.config.missing_env);
        }
        ...
        let mut params: Vec<(String, String)> = vec![
            ...
            ("line_items[0][price_data][unit_amount]".into(),                     request.amount_cents.to_string()),
            ("line_items[0][price_data][product_data][name]".into(),              video_title),
            ("line_items[0][quantity]".into(),                                    "1".into()),
            ("success_url".into(), format!("{}?session_id={{CHECKOUT_SESSION_ID}}", success_url)),
