...
/// Meminta izin & men-setup sesi HLS dengan watermark burn-in (posisi acak periodik).
pub async fn request_play(
    ...
    // 4) Ambil username untuk watermark
    ...
    // 6) Transcode → HLS + watermark burn-in (posisi acak tiap MOVE_INTERVAL detik)
    let seg_secs = st.cfg.hls_segment_seconds.max(2) as u32;
    let master_path = session_dir.join("master.m3u8");

    const MOVE_INTERVAL: i32 = 5; // detik
    const PADDING: i32 = 20;      // px
    let font_path = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf";

    // ====== Watermark text → tulis ke file agar tidak diparse sebagai textfile implisit
    // Gunakan strftime agar timestamp realtime (%Y-%m-%d %H:%M:%S)
    ...
    // ====== Posisi acak stabil per interval n=floor(t/interval)
    ...
    // ====== drawtext utama + shadow; gunakan textfile= & expansion=strftime
...
fontsize=20:fontcolor=white@0.15:box=0",
    ...
    if let Err(e) = transcode_hls(&v.filename, session_dir.to_string_lossy().as_ref(), args).await {
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
...
/// Sajikan file HLS dari hls_root/<session>/...
pub async fn serve_hls(
    ...
    match fs::read(&fp).await {
        Ok(bytes) => {
            ...
            headers.insert(
                header::CONTENT_TYPE,
                HeaderValue::from_static(match file_type(&file) {
...
enum FileType { M3U8, TS, MP4, Unknown }
...
fn is_safe_file(s: &str) -> bool {
    !s.is_empty()
        ...
        && !s.contains('\\')
        && !s.contains("..")
        && (s.ends_with(".m3u8") || s.ends_with(".ts") || s.ends_with(".mp4"))
}
